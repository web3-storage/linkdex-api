...
export interface Reporter {
  report (cid: UnknownLink): Promise<Report>
}

export interface Report {
  /** Keys for CAR files where the DAG may be found. */
  cars: string[]
...
export class RawReporter implements Reporter {
  ...
  async report (cid: UnknownLink): Promise<Report> {
    ...
    for (const carKeys of carGroups) {
      ...
      if (index.getDagStructureLabel() === 'Complete') {
        ...
        return { cars: carKeys, ...index.report(), structure: 'Complete' }
      }
      reports.push({ cars: carKeys, ...index.report(), structure: index.getDagStructureLabel() })
    }
...
export class CompleteReporter implements Reporter {
  ...
  async report (cid: UnknownLink): Promise<Report> {
    ...
    return { cars: [key], ...index.report(), structure: index.getDagStructureLabel() }
  }
}
